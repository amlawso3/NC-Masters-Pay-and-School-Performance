...
"""
Author:       Ashley Avis
Filename:     Creating School Data Clean
...
Looking to analysis, we expect and will investigate the following about the data:
    - Numeric data are likely not normal
    - Variables are likely highly correlated
    - There are likely many features that have a relationship with school performance scores, so we'll
      need to narrow them down accounting for collinearity and interpretability
...
#%% School info - This will be the base file for merging everything together
...
#%% School Performance Grades - Outcome measure for main analysis
...
def grad_clean(grad, yr):
    ...
    # I assumed values of 95, 5, or 0 in these instances
    grad['GradRate' + str(yr) + 'Yr'].replace(['>95', '<5', '*'], [96, 4, 0], inplace=True)
    # Drop of LEA info
...
total=total[total['spg_score'].notnull()]
